...
class Rotor:
    ...
    def get(self, index):        #index = one letter from message text
        return self.values[index]
...
class Enigma:
    def __init__(self, rotors_number, base):   #base = BASE =256
    ...
    def read_config(self, rotors_config, config_base):    #when descryption checke after encrypt roter states and init                               ial  time state
    ...
    def encrypt_one(self, byte):
        result = byte                            #one letter in text file message
        for rotor in self.rotors:
    ...
    def encrypt(self, byte_list):  #byte_list = data from text file
        result = b"" # result into bytes
